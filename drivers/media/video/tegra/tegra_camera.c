...
{
    if (!tegra_camera_dev) return -1;

    if (on){
        printk("camera mclock on\n");
        clk_set_rate(tegra_camera_dev->csus_clk, 6000000);
        clk_set_rate(tegra_camera_dev->vi_sensor_clk, 24000000);
        clk_enable(tegra_camera_dev->csus_clk);
        clk_enable(tegra_camera_dev->vi_sensor_clk);
    }
    else{
        clk_disable(tegra_camera_dev->vi_sensor_clk);
        clk_disable(tegra_camera_dev->csus_clk);
    }
printk("-%s\n",__FUNCTION__);
    return 0;
}
